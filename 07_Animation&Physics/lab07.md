...
Importing a `.fbx` model with animations will automatically create an `AnimationPlayer` node. This node can play a single animation at a time and has parameters such as `Speed Scale`. You can use the `Current Animation` property to try out and **play animations** of the player model.
...
> aside positive
> More complex behaviors, such as **blending animations**, **changing them** based on current circumstances/parameters, need to be done either by **`AnimationTree`** node (covered in a later section) or **your own implementation** for switching animations.
...
In this section, I would like to tell you the theory behind **Skeletal Animations**, how they work, and how they are created. We will look at the process of creating a proper **Rig** for our player model. Feel free to look over this section if you are interested, or you can skip it and come back later if some part of **Skeletal Animation** is unclear.

**Skeletal Animation** is a technique of using connected **bones** to simulate movement and bend the vertices of the model in natural ways. **Animations** are then created by only moving the bones of the model, since the vertices follow the bones.

### Step 1: Skeleton
First, the bones of the model need to be created and connected. The skeleton does not need to match all the real joints and bones that the model has. For example, here you can see the **Skeleton** of our player model, which only has one bone for the whole right arm, since I do not want the elbow to bend in any of the animations:
...
1. **Add** an `AnimationTree` node as a child of the `Mesh` node.
...
### State machine
With the state machine open, we can see the `Start` node, which is the initial state, and the `End` node. The state machine can be used as such:
...
- You can connect nodes by **left-clicking and dragging** the white circles
...
> aside positive
> You can use the first tool, as I did in the GIF, to try out how different values of the 2D blending parameter look.

> aside positive
> I created 4 animations to blend between, but the blend tree can use any number of animations (diagonal movement, for example) to create a better looking walking.
...
This makes the `AnimationTree` work directly with the variable of `_connected`, which is set when the player enters and exits the 2D puzzle.
...
Now, to make the lights follow the eyes, we will attach them to a bone like this:
...
Now the only thing left to do is to move the actual `GunTarget` based on the direction the camera is looking. I already prepared the logic in the code, since I want it to be a bit more complex than just moving the target.
...
### Player Projectiles
Now that our gun rotates to where we are looking, it would be nice to actually **shoot a projectile**. I have already prepared a scene with the projectile (`3D/Player/Projectile`), which can be easily instantiated. We first need the position and rotation of the tip of the gun.
...
- Set the **velocity** based on the direction of the gun and the projectile speed from the player's stats
...
### Enemy Damage
The shooting is nice, but in the current state, it is useless, since the enemies **do not receive damage**, so let's change that. There are many ways to do this, but since I want the game to feature breakable walls, boxes, etc., we will use a **group** (same as tag in Unity) to denote which objects can receive damage.
...
### Result
To preview the `SkeletonIK`, it is necessary to click the `Play IK` button on the top bar of the scene view. Here is the resulting behavior:

![](img/SkeletonIKShowcase.gif)

Of course, you can then set rotation limits for the bones by implementing your own `SkeletonModifier3D`, so that they only rotate realistically.
...
1. **Add** an `AnimationPlayer` node as a child of the `AnimatedPlatform`
...
You can add another track for rotation (`Bezier Curve`) to make the animation a bit more interesting. I first set the keyframes the same way as we did with the `position`, and then I switched to the **bezier curve editor** to make the animation a bit more interesting.
...
> 1. Try the newest version of Godot to see if it is already fixed
> 2. Keep `Sync To Physics` off, making the player motion a bit jittery. 
...
> aside positive
> Another possibility is to also take into account the velocities of the player and rigidbody to create a more accurate simulation and to make the rigidbodies also push the player. However, if you want to create a physics-based game (`Fall Guys`, `Human: Fall Flat`, `Gang Beasts`, etc.), it would be easier if the player were of node type `RigidBody3D`.

#### Position Offset
The next thing, before we apply the force, is the offset from the rigidbody center to the collision point. We need this offset for the `apply_impulse()` function, which will be used in the next step. Here is the line to get the offset:
...
Having different physics objects with different physical properties is essential. The `RigidBody3D` node has these properties that can be changed:
...
This section will look at one type of **physics joints**, which can be used to create objects such as seesaws, doors, and other objects with similar axes of movement. We will make the seesaw object I modelled behave in a physically correct way.
...
#### Convex collider
This collider is a tradeoff between precision and performance. It still creates a **precise collider** based on the underlying mesh with one special property. The resulting collider is a **convex shell**, or in other words, you can imagine the object tightly wrapped in plastic wrap.
...
#### Composite collider
A composite collider is not really a collider type. It is more like a **method of creating colliders** from simple collision shapes.
...
### Axis Lock
Now, with a proper collider, we can implement the behavior of the seesaw. The first way to do it is using the `Axis Lock` functionality. I locked all the axes before the codelab so that the `SeeSaw` would not fall through the ground.
...
Here is how the seesaw should behave now. As you can see in the GIF, the seesaw works better when the `Mass` parameter is set to `10`. I recommend keeping the `Mass = 10`.
...
#### Angular Limits
The rotation along the joint axis can then be further **limited** to set minimum and maximum angles. Many parameters can be set to tweak the behavior, which can make the joint more or less stable:

![](img/JointParams.png)

Try to **play around** with the angle limits to see how they affect the behavior of the `SeeSaw`.
...
Many other joint types can be used. The selection of `2D` and `3D` joints also differs a bit due to the dimension specifics.
...
#### `HingeJoint3D`
- Already explained in great detail in the previous section.
...
> aside positive
> Feel free to use this script in your own projects if you need to use the `Generic6DOFJoint3D` node for something.
...
The motor option makes the rigidbody move in a set direction. We want the platform to have a stronger spring on the up `Y-axis`, so set the motor to:
...
The spring option makes the rigidbody behave like being attached to a spring. We want the platform to work as a spring on the `Y-axis`:
...
- We looked at the 3 types of animations:
    - **Premade** - used the `AnimationTree` node and created a complex animator behavior (`StateMachine`, `BlendTree`, `Timescale`, etc.).
