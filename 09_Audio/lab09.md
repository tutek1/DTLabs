...
Here is the template for this lab. Please download it. There are scripts, audio clips, and scenes required for the implementation of audio playback.
<button>
...
Let's first create a simple audio player without the use of the `AudioManager`. We will use it for playing ambient computer noises near the 2D puzzle section.
...
> aside positive
> Many more parameters can be set, for example, the Doppler Effect. I won't go over all of them, since most of them are very straightforward and can be looked up in the documentation.
...
#### Methods
There are several implemented and ready-to-implement methods.
...
- `play_music()` - starts playing music with a given fade-in time
- `stop_music()` - stops playing music with a given fade-in time


### Play Music
Let's now complete the implementation of the music player so that we can test it out. Since there will only ever be one type of music playing in our game, we will use a single `AudioStreamPlayer` as a child of the `AudioManager`, which will be reused.

1. **Add** an `AudioStreamPlayer` node as a child of the `AudioManager`
...
- All three functions will be very similar. They will only differ by a few lines.

> aside positive
> Some of you might say that creating an `AudioStreamPlayer` node for each played SFX and destroying it after the sound stops is wasteful. I agree with you, and in a different game that plays a lot of SFXs, a better approach, such as pooling of `AudioStreamPlayer` nodes, should be used. However, the performance impact should not be noticeable if you keep the number of played SFX under 100 per second (rough estimate).
...
### Player Walk
The walking of the player is a bit trickier. There are several ways to do it.
...
Here, we want to use the `play_sfx_at_location` with the location of the collectible and not set it as the child of the collectible since it will be destroyed before the sound finishes playing.
...
### Result
Now that all the sounds that I prepared are being played, let's see what the result looks like. Play the game or watch the following video:
